                ...
                <div class="table-responsive p-0">
                    <table class="table align-items-center mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    Container No.</th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Size
                                </th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Agent
                                </th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    Container Owner</th>
                        ...
                        <tbody>
                                ...
                                <tr>
                                    <td class="text-center">
                                        <div class="form-check d-flex justify-content-center"><input
                                                class="form-check-input container-checkbox" type="checkbox" name="ids[]"
                                                value="{{ $container->id }}"></div>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0 px-2">{{ $container->container_no }}</p>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0 px-2">{{ $container->size }}</p>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0 px-2">{{ $container->agent }}</p>
                                    </td>
                                    @if ($container->container_owner == 0)
                                        <td>
                                            <p class="text-xs font-weight-bold mb-0 px-2">Rental</p>
                                        </td>
                                    @elseif ($container->container_owner == 1)
                                        <td>
                                            <p class="text-xs font-weight-bold mb-0 px-2">Owner</p>
                                        </td>
                                    @endif
                                    <td class="align-middle text-center">
                                        ...
                                        @can('delete containers')
                                            <button type="button" class="btn btn-link text-danger mb-0" data-bs-toggle="modal"
                                                data-bs-target="#deleteModal-{{ $container->id }}" title="Delete"><i
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center p-3">No containers found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            containerCheckboxes.forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    selectAllCheckbox.checked = Array.from(containerCheckboxes).every(cb => cb
                        .checked);
                    toggleBulkDeleteBtn();
                });
            });
