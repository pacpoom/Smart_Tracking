                ...
                <table class="table align-items-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Location Code</th>
                            ...
                            <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Bin</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-center"><div class="form-check d-flex justify-content-center"><input class="form-check-input location-checkbox" type="checkbox" name="ids[]" value="{{ $location->id }}"></div></td>
                            <td><p class="text-xs font-weight-bold mb-0 px-2">{{ $location->location_code }}</p></td>
                            {{-- ดึงชื่อ Location Type จากตาราง yard_categories ผ่าน Relationship ที่ชื่อ locationType --}}
                            <td><p class="text-xs font-weight-bold mb-0 px-2">{{ $location->locationType?->name ?? 'N/A' }}</p></td>
                            {{-- ดึงชื่อ Zone จากตาราง yard_categories ผ่าน Relationship ที่ชื่อ zone --}}
                            <td><p class="text-xs font-weight-bold mb-0 px-2">{{ $location->zone?->name ?? 'N/A' }}</p></td>
                            {{-- ดึงชื่อ Area จากตาราง yard_categories ผ่าน Relationship ที่ชื่อ area --}}
                            <td><p class="text-xs font-weight-bold mb-0 px-2">{{ $location->area?->name ?? 'N/A' }}</p></td>
                            {{-- ดึงชื่อ Bin จากตาราง yard_categories ผ่าน Relationship ที่ชื่อ bin --}}
                            <td><p class="text-xs font-weight-bold mb-0 px-2">{{ $location->bin?->name ?? 'N/A' }}</p></td>
                            ...
                            <td class="align-middle text-center">
                                @can('edit yard locations')
                                    <a href="{{ route('yard-locations.edit', $location->id) }}" class="btn btn-link text-secondary mb-0" title="Edit"><i class="material-symbols-rounded">edit</i></a>
                                @endcan
                                @can('delete yard locations')
                                    <button type="button" class="btn btn-link text-danger mb-0" data-bs-toggle="modal" data-bs-target="#deleteModal-{{ $location->id }}" title="Delete"><i class="material-symbols-rounded">delete</i></button>
                                @endcan
                            </td>
                        </tr>
                        @include('yard-locations.partials.delete-modal', ['location' => $location])
                        @empty
                        <tr><td colspan="8" class="text-center p-3">No locations found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        locationCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function () {
                selectAllCheckbox.checked = Array.from(locationCheckboxes).every(cb => cb.checked);
                toggleBulkDeleteBtn();
            });
        });
        toggleBulkDeleteBtn();
